...
import { MockOperator } from "../typechain-types";
import { HardhatEthersSigner } from "@nomicfoundation/hardhat-ethers/signers";

describe("MockOperator Contract", function () {
  ...
  beforeEach(async function () {
    ...
    const MockOperatorFactory = await ethers.getContractFactory("MockOperator");
  ...
  describe("Operator Test", function () {
    ...
    it("5 - Should revert on zero address", async function () {
      ...
      ).to.be.revertedWithCustomError(mockOperator, "ZeroAddressGivenForNewOperator");
    });
    ...
    it("7 - Should revert onlyOperator function for non-operator", async function () {
      ...
      ).to.be.revertedWithCustomError(mockOperator, "CallerIsNotOperator");
    });
